...
class AnalizerDelu(Analizer): 
    ...
    def urlGenerator(self):
        #dias = abs(datetime.now() - self.__date).days
        dias = 50
        print("Dias que han pasado: ")
        print(dias)
        paginas = (dias // 10) + 1
        ...
        for url in indices:
            try: 
                ...
                for linea in lineas:
                    ...
                    print("Encontrando el dato")
                    t_date2 = celdas[1].getText().split()[2].split("/")
                    fecha = date(int(t_date2[2]),int(t_date2[1]),int(t_date2[0]))
                    print(fecha)

                    if (self.__date.month == fecha.month and self.__date.day == fecha.day):
                        print("Se ha encontrado la fecha")
